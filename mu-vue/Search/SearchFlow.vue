...
<script>
  ...
  import browser from '../../mu/browser'
  ...
  export default {
    ...
    created: function () {
      this.history = browser.localStorage('mu-history').split(',').filter((item) => {
        if (item) return item
      })
    },
    components: {
    ...
    methods: {
      ...
      search: function () {
        if (this.value) {
          let hist = browser.localStorage('mu-history').split(',').filter(item => item)
          if (hist.indexOf(this.value) === -1) {
            hist.unshift(this.value)
            this.history = hist
            browser.localStorage('mu-history', hist.join(','))
          }
          this.value = ''
        }
      }
